...
def test_gradients(varen_model, pose_cpu, shape_cpu, global_orient_cpu, translation_cpu):
    """Test to see if gradients are computed correctly."""
    def make_input(tensor):
        noise = torch.rand_like(tensor, dtype=tensor.dtype, device=tensor.device)
...
def test_gradients_without_muscles(varen_model_no_muscles, pose_cpu, shape_cpu, global_orient_cpu, translation_cpu):
    """Test to see if gradients are computed correctly."""
    def make_input(tensor):
        noise = torch.rand_like(tensor, dtype=tensor.dtype, device=tensor.device)
        return (tensor + noise).detach().requires_grad_()
...
def test_faces(varen_model):
    ...
    assert faces.ndim == 2, "Faces should be a 2D tensor"
    assert faces.shape[1] == 3, "Faces should have 3 vertices per face"
    assert faces.dtype == np.int64, f"Faces should be of type int64, got {faces.dtype}"
    assert faces.shape[0] > 0, "Faces tensor should not be empty"


def test_model_initialization_with_default_values(varen_model_dir, varen_model_file_name, ckpt_file):
    ...
    assert varen_model.betas.shape == (BATCH_SIZE, NUM_BETAS), "Betas shape should be (2, 15)."
    assert varen_model.global_orient is not None, "Global orientation should not be None."
    assert varen_model.global_orient.shape == (BATCH_SIZE, 3), "Global orientation shape should be (2, 3)."
    assert varen_model.body_pose is not None, "Body pose should not be None."
    assert varen_model.body_pose.shape == (BATCH_SIZE, varen_model.NUM_JOINTS * 3), f"Body pose shape should be ({BATCH_SIZE}, {varen_model.NUM_JOINTS * 3}. Got {varen_model.body_pose.shape})."
